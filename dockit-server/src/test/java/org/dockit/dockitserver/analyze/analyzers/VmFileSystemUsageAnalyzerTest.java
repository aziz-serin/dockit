...
public class VmFileSystemUsageAnalyzerTest {
    private static final String ALERT_FS_NAME = "alertFs";
    private static final String NO_ALERT_FS_NAME = "noAlertFs";


    @Autowired
    private VmFileSystemUsageAnalyzer vmFileSystemUsageAnalyzer;
    ...
    public void analyzeReturnsAlertGivenImportantCpuLoad() {
        List<Alert> alerts = vmFileSystemUsageAnalyzer.analyze(alertAudit);

        assertThat(alerts.size()).isEqualTo(1);
        assertThat(alerts.get(0).getImportance()).isEqualTo(Alert.Importance.CRITICAL);
        assertThat(alerts.get(0).getMessage()).containsIgnoringCase(ALERT_FS_NAME);
    }

    @Test
    public void analyzeDoesNotReturnAlertGivenUnImportantCpuLoad() {
        List<Alert> alerts = vmFileSystemUsageAnalyzer.analyze(noAlertAudit);
